...
void main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		// CHECK FOR CHANGING DIRECTORY COMMAND
		if (strncmp(cmd, "cd", 2) == 0)
		{
			if (strncmp(cmd+3, "~", 1) == 0)
				chdir(getenv("HOME"));
			else
				chdir(cmd + 3); // POINTER GETS ADDRESS OF CMD + 3
			continue;			// 0=[c] 1=[d] 2=[\0] 3=[PATH]
		}
		...
		if ((args = (char**)malloc((args_cnt) * sizeof(char *))) == NULL)
			...
			for (i = 1, args[0] = cmd, cmd_ptr = cmd; i < args_cnt - 1; cmd_ptr++)
				if (*cmd_ptr == '\0')
					args[i++] = cmd_ptr + 1;
		args[args_cnt - 1] = NULL; // LAST POINTER NULL (fgets())
		...
		wait(&stat);   // PARENT: WAIT FOR CHILD PROCESS TO END
